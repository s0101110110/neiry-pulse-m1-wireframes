...
## Pre-flight check ПЕРЕД любой UI-задачей (HARD GATE)

Перед тем как открыть `Edit` / `Write` на HTML/CSS файле — **обязательно остановиться** и пройти чек-лист:

1. **Это UI-задача?** Признаки UI-задачи:
   - HTML / CSS / SVG правки
   - Создание/изменение визуальных мокапов или превью
   - Slicing, PNG crop, transparent-фоны
   - Применение или сравнение шрифтов / цветов / компонентов
   - Любая работа со skills (`emil-design-eng`, `impeccable`, `redesign-existing-projects`)

2. **Если ДА (любой пункт выше) — НЕ ДЕЛАТЬ САМ.** Действия:
   - Написать бриф в `UI_assets/BRIEF.md` (что/зачем/защищённые элементы/ожидаемый выход)
   - Если нужен skill — указать в брифе («Skill `impeccable args:"critique <path>"`»)
   - Запустить UX/UI агента через Agent tool с `cwd: UI_assets/`
   - Получить результат, прочитать, показать PM скриншот, ждать «принято»
   - Только после acceptance — коммит

3. **Исключения, когда PM может сам:**
   - Однострочные правки в markdown (PRD/ТЗ/roadmap)
   - Правка YAML / config / settings
   - Текстовые документы без визуальной составляющей

4. **Что говорит этот gate "нет":** «быстрее самому», «это же мелочь», «всё равно покажу PM до коммита». Эти аргументы — красные флаги, что я нарушаю правило. Дисциплина важнее скорости.

5. **Память-якорь:** `feedback_neiry_pm_role_boundary.md` — критическое правило о границах роли PM.

**Self-check вопрос:** *«Если бы я был ревьюером своих действий — увидел бы я нарушение PM-границы?»* Если ответ «возможно, да» — стоп, делегировать.

## НЕЗЫБЛЕМОЕ ПРАВИЛО — Visual Solution Always Includes Path

**Каждый раз** когда я даю PM **визуальное решение** (screenshot, HTML мокап, графический артефакт, любой UI-output) — **ПОСЛЕДНЕЙ СТРОКОЙ** сообщения должен быть **прямой путь** где это смотреть.

**Что считается visual solution:**
- Screenshot в чате через Read tool
- Generated HTML preview/mockup
- Refactored screen
- New design artefact (cheatsheet, gallery, comparison page)
- Любой PNG/JPG/SVG output для PM-review

**Формат пути (всегда последней строкой):**
- Абсолютный filesystem path: `/Users/solomono/.../file.png`
- Или GitPages URL если запушено: `https://s0101110110.github.io/...`
- Или `/tmp/preview.png` для временных артефактов
- Множественные пути — списком, но в конце сообщения, не разбросанно

**Почему правило незыблемо:**
- PM физически не может оценить визуал в чате если не знает где смотреть полную версию
- Чат-превью часто сжаты/обрезаны → нужен оригинал
- Каждое нарушение этого правила = время PM на «где?» вместо ревью

**Self-check перед отправкой visual-сообщения:**
*«Есть ли в последней строке прямой путь / URL где PM может это посмотреть?»* Если нет — **остановиться, дописать путь, потом отправить.**

**Память-якорь:** `feedback_neiry_pm_visual_path_required.md`.
