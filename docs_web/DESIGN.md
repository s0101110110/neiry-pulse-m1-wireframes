...
description: "Neiry Pulse — мобильное приложение био-мониторинга (Android + iOS, 390×844). Дизайн-направление — Bevel-clone v3: прохладно-серый surface (#F0F0F3) + белые карточки на мягких тенях, мультисемантическая палитра (green/orange/red severity), light-mode primary. Wine #831843 — единственный хроматический акцент (бренд-якорь), живёт в bottom-nav active + chip-badge. Типографика: Golos Text (заголовки + UI/body) + Pulse Mono (все числа и табличные данные). Сетка 4px. Этот файл — YAML-зеркало docs_web/bevel-clone/shared/tokens.css (--bv-* → bv-*) для Stitch / AI-tools. Dark Mode отложен на Sleep-раздел."
...
typography:
  ...
  sizes:
    ...
    headline: 16px             # app-bar-title · device-name (v1.0.4: 17→16)
    label: 12px                # mcard-label · row-label (v1.0.4: 14→12)
    body: 14px
    caption: 12px
    eyebrow: 11px
...
components:
  ...
  bv-nav-item:
    ...
    is-active: "color {colors.bv-wine} на icon+label, weight 600 — без chip-bg (PM Stage 5)"   # WINE spot #1
  bv-avatar:
...
**Source-of-truth для CSS:** `docs_web/bevel-clone/shared/tokens.css` (CSS-переменные `--bv-*`) + `components.css` (компонентные классы, shared). Этот DESIGN.md — их **YAML-зеркало** для Stitch / AI-tools: значения совпадают 1:1, ключи `bv-*` соответствуют `--bv-*`. Эталонные экраны: `bevel-clone/home/home-main-etalon.html`, `home/home-health-monitor.html`, `hrv/hrv-detail.html`, `settings/*`.
...
> Source-of-truth: `shared/tokens.css`. Палитра принята PM 26–29.06 (cool-gray v3 + multi-semantic), memory `project_neiry_bevel_clone_v3`.
...
1. **bottom nav-active** (`bv-nav-item.is-active`) — wine color на icon + label, weight 600 (без chip-bg, PM Stage 5).
...
| `{typography.sizes.headline}` | 16px | app-bar-title · device-name (Golos 700) |
| `{typography.sizes.label}` | 12px | mcard-label · row-label (Golos 500) |
...
Первая волна DS — 14 атомарных `bv-*` (scope утверждён PM 29.06, memory `project_neiry_ds_wave1_approved`). Превью: `shared/components-preview-DRAFT.html`.
...
- `bv-nav-pill` — floating solid-white 5-tab (shadow-nav); `bv-nav-item.is-active` — **wine color на icon+label, weight 600, без chip-bg** (главный wine-якорь, PM Stage 5).
- **Tabs-канон (5):** «Дом · Тренировка · История · Здоровье · Ещё» — единый набор на ВСЕХ Home-экранах (PM 29.06).
- **Sub-screen nav-hide:** на settings sub-screens (bt-pairing / notifications / privacy) bottom nav-pill **скрыт** — навигация только через `bv-app-bar--back` (back-arrow). Bevel-канон.
...
- `version: v1.0.3` — зеркало `tokens.css` v1.0.3 (73 переменных). v1.0.1: ink-dim AA-fix · toggle-track-off · section-h2 24px. v1.0.2: +r-button 14 · +r-icon 10. v1.0.3: +wine-hover · +ink-on-wine · +shadow-dot; Wine Policy → 7 зон; chip--badge → wine-tint; nav-pill → solid. v1.0.4: label 14→12 · headline 17→16; nav-active без chip-bg; +tabs-канон 5; +sub-screen nav-hide (Stage 5 visual review). Stage 1 commit a958cc0.
- **Source-of-truth CSS:** `docs_web/bevel-clone/shared/tokens.css` (+ `components.css` — Stage 3).
